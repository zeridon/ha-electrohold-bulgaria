...
The integration automatically determines whether the current tariff is Дневна (Day) or Нощна (Night) according to Electrohold's published seasonal schedule.

# Features
 * Retrieves Electrohold regulated electricity prices automatically.
 * Checks the Electrohold website once every 24 hours.
...
The tariff calculation is performed locally in Home Assistant. The Electrohold website does not need to be contacted when the tariff changes.
...
Electricity prices are retrieved from the Electrohold Bulgaria regulated-price page:

https://electrohold.bg/bg/sales/domakinstva/snabdyavane-po-regulirani-ceni/

The integration uses the final regulated electricity price including network services but excluding VAT as published by Electrohold.

# Disclaimer

This is an unofficial Home Assistant integration and is not affiliated with or endorsed by Electrohold Bulgaria.

Always verify electricity prices against your electricity bill and the current information published by Electrohold.
